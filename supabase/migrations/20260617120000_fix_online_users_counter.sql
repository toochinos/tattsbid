-- Fix online users counter: server UTC last_seen, count RPCs, cleanup bad rows.

-- 1. Server UTC on every write (never trust client clock).
...
-- 2. Heartbeat upsert (Flutter calls this every 30s).
...
-- 4. Online count: last_seen within last 60 seconds (server UTC), not in the future.
...
-- Alias some SQL editors / docs use.
...
-- 5. Active today: last 24 hours, not in the future.
...
-- 6. One-time cleanup of bad rows (future client timestamps).
delete from public.online_users
where last_seen > timezone('utc', now()) + interval '1 minute';

-- Verify:
--   select public.count_online_users();
--   select public.get_online_users_count();
