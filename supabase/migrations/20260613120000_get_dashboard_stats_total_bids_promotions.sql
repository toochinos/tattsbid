-- Extend get_dashboard_stats with total_bids and total_promotions.
-- Does not modify count_online_users, count_active_users_today, or presence helpers.
--
-- Schema note: there is no public.promotions table. Artist promo posts on Explore
-- are tattoo_requests whose owner profile has user_type = tattoo_artist.
...
comment on function public.get_dashboard_stats() is
  'Developer dashboard aggregates. total_bids = count(bids). total_promotions = artist tattoo_requests. Online/active via count_online_users / count_active_users_today.';
