-- get_dashboard_stats(): all counts inline — no count_online_users / count_active_users_today deps.
...
begin
  ...
  select count(distinct user_id)::bigint into v_active_users_today
  from public.online_users
  where last_seen > v_now - interval '24 hours';

  select count(distinct user_id)::bigint into v_users_online_now
  from public.online_users
...
comment on function public.get_dashboard_stats() is
  'Developer dashboard aggregates; inline counts only (no helper RPCs).';

revoke all on function public.get_dashboard_stats() from public;
grant execute on function public.get_dashboard_stats() to authenticated;

-- Verify:
--   select jsonb_pretty(public.get_dashboard_stats());
--   select (public.get_dashboard_stats()->>'total_users')::int;
--   select (public.get_dashboard_stats()->>'total_bids')::int;
